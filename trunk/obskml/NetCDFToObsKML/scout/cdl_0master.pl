...
print( "Processing: $net_cdf_file---------------------------------------------------------------------------------------------------------\n" );
print( "Command Line Args: net_cdf_file: $net_cdf_file FileType=$FileCreationOption ObsKMLPath=$strObsKMLFilePath LastNTimes=$iLastNTimeStamps\n" );
#
...
if (!($format_category_value eq 'fixed-point')
  ...
  && !($format_category_value eq 'fixed-map')) {die "ABORT! $format_category_value is not a supported format.\n";}
...
if ($format_category_value eq 'fixed-point') {
  ...
  # Added parameters $FileCreationOption and $strObsKMLFilePath.
  #DWR v1.1.0.0 6/2/2008
  #Added use of $iLastNTimeStamps.
  #fixed_point_1($this_station_id_top_ts, $FileCreationOption, $strObsKMLDir, $iLastNTimeStamps );
...
sub fixed_point_1 () {
  ...
  for $i (0..$#air_pressure) {
    ...
    if ($air_pressure[$i]{'dim_id'} != $time_dim{'dim_id'}) {
      ...
      #DWR 4/5/2008
      $air_pressure[$i]{'units'} = $units_value;
	...
	if ($this_air_pressure_data[$j] == $missing_value_value || $this_air_pressure_data[$j] == $Fill_value_value || $this_air_pressure_data[$j] > 900) { $this_slope = 1; $this_intercept = 0; } else { $this_slope = 1000; $this_intercept = 0;} 
print STATION_ID_SQLFILE "-- this_air_pressure_data[j]=".$this_air_pressure_data[$j]." missing_value_value=".$missing_value_value." Fill_value_value=".$Fill_value_value."\n";
  ...
  for $i (0..$#sea_surface_eastward_current) {
    ...
    if ($sea_surface_eastward_current[$i]{'dim_id'} != $time_dim{'dim_id'}) {
      ...
      #DWR 4/5/2008
      $sea_surface_eastward_current[$i]{'units'} = $units_value;
  ...
  for $i (0..$#sea_surface_northward_current) {
    ...
    if ($sea_surface_northward_current[$i]{'dim_id'} != $time_dim{'dim_id'}) {
      ...
      $base_units->convert($dest_units,$this_slope,$this_intercept);
      #DWR 4/5/2008
      $sea_surface_northward_current[$i]{'units'} = $units_value;
          ...
          if( $this_time_stamp_sec > $oldest_ok_timestamp )
          {
            if ($water_level[$i]{'data'}[$j] != $missing_value_value
              ...
              ) 
              {
                ...
                $DataVal = $water_level[$i]{'reference'};
      ...
      for my $i (0..$#significant_wave_height) {
        for my $j (0..$#this_significant_wave_height_data) {
          ...
          if ($significant_wave_height[$i]{'data'}[$j] != $missing_value_value
            ...
            if ($significant_wave_height[$i]{'data'}[$j] == $missing_value_value
              || $significant_wave_height[$i]{'data'}[$j] == $Fill_value_value
              || $significant_wave_height[$i]{'data'}[$j] == '') {
              print WAVE_SQLFILE ',NULL';
            }
            else {
