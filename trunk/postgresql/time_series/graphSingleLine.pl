...
my ($dbh,$sth,$sql);
if ($db_host eq '') { $dbh = DBI->connect ("dbi:Pg:dbname=$db_name", "$db_user", "$db_passwd"); } #remove host reference if local
else { $dbh = DBI->connect ("dbi:Pg:dbname=$db_name;host=$db_host", "$db_user", "$db_passwd"); }
...
$sql = qq{ select long_name,url from organization where row_id = $organization_id };
#print "sql:".$sql."\n";
$sth = $dbh->prepare( $sql );
$sth->execute();
if ($sth->rows == 0) { &exit_no_data; }
...
#if ($time_query eq 'time_last') { $sql = qq{ SELECT TO_CHAR((m_date - interval '$time_subtract hours'), 'MM-DD-YYYY hh24:mi'), $column_value FROM $db_table where sensor_id = $sensor_id and date(m_date) > date(current_timestamp - interval '$time_interval') and $column_value >= $range_min and $column_value <= $range_max $qc_clause order by m_date }; }
#elsif ($time_query eq 'time_date') { $sql = qq{ SELECT TO_CHAR((m_date - interval '$time_subtract hours'), 'MM-DD-YYYY hh24:mi'), $column_value FROM $db_table where sensor_id = $sensor_id and m_date >= timestamp '$from_date 00:00:00' + interval '$time_subtract hours' and m_date <= timestamp '$to_date 00:00:00' + interval '$time_subtract hours' and $column_value >= $range_min and $column_value <= $range_max $qc_clause order by m_date }; }

#the below should be faster queries making use of the multi_obs index
if ($time_query eq 'time_last') { $sql = qq{ SELECT TO_CHAR((m_date - interval '$time_subtract hours'), 'MM-DD-YYYY hh24:mi'), $column_value FROM $db_table where sensor_id = $sensor_id and m_date > date(current_timestamp - interval '$time_interval') and $column_value >= $range_min and $column_value <= $range_max $qc_clause and m_type_id = $m_type_id order by m_date }; }
elsif ($time_query eq 'time_date') { $sql = qq{ SELECT TO_CHAR((m_date - interval '$time_subtract hours'), 'MM-DD-YYYY hh24:mi'), $column_value FROM $db_table where sensor_id = $sensor_id and m_date >= timestamp '$from_date 00:00:00' + interval '$time_subtract hours' and m_date <= timestamp '$to_date 00:00:00' + interval '$time_subtract hours' and $column_value >= $range_min and $column_value <= $range_max $qc_clause and m_type_id = $m_type_id order by m_date }; }
