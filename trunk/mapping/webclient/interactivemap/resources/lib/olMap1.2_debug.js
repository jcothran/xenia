...
/*
rcoosmapping.radioButtonPluginEx = Ext.extend(GeoExt.tree.RadioButtonPlugin,
{
  onRenderNode: function(node) {
     var a = node.attributes;
     if(a.radioGroup && !a.radio) {
         if(!node.layer.options.QUERYSELECTED) {
    ...
    if(dataPanel !== null)
    {
      ...
      if(parent !== null)
      {
        ...
        if(rec !== null)
        {
          ...
          //Get the popup panel so we can get the platform feature information.
          var eventObj = {
            'platformName': parent.feature.get("staID"),
            'obsName' : rec.get('obsDisplayName'),
            'uom' : rec.get('uomName'),
            'data' : timeSeries
          };
          this.fireEvent("obsselected",eventObj);
...
/*------------------------------------------------------------------------------------------------------------------------------------*/
rcoosmapping.kmlPopup = Ext.extend(GeoExt.Popup,{
  feature : undefined,
  ...
  constructor: function(config) 
  {
    ...
    rcoosmapping.kmlPopup.superclass.constructor.call(this,config);
  },
  ...
  */
  ...
  {
    var count;
    if(this.feature.cluster != undefined)
    {
      var tabs = [];
      count = this.feature.attributes.count;
      for(var i = 0; i < count; i++)
      {
        var content = this.buildContent(this.feature.cluster[i]);    
...
rcoosmapping.kmlLayer  = Ext.extend(OpenLayers.Layer.Vector,{
  ...
  kmlClick: function(feature)
  {
    ...
    this.popup = new rcoosmapping.kmlPopup({
        id: 'kmlPopup',
        map: this.mapObj,
        title: "Information",
        autoScroll: true,
...
/*------------------------------------------------------------------------------------------------------------------------------------*/
  ...
  GeoExt.tree.layerLoaderExtended = Ext.extend(GeoExt.tree.LayerLoader, {
    ...
    */
    ...
    createNode: function(attr) {
      ...
      if((this.defaultGFILayer.length > 0) && node.layer.name == this.defaultGFILayer)
      {
  ...
  rcoosmapping.olMap = Ext.extend(Ext.Panel,
  {
    ...
    createLayer : function(layerType, name, url, params, options, creategetFeatureInfoClick) 
    {
      ...
      else if(layerType == "OpenLayers.Layer.WMSEx")
      { 
        var layer = new OpenLayers.Layer.WMSEx(name, url, params, options);
        if(creategetFeatureInfoClick)
        {
          this.obsInfoControls.push({
          click: new OpenLayers.Control.WMSGetFeatureInfo(
            ...
            new GeoExt.tree.RadioButtonPlugin({ 
                ...
                { 
                  ...
                  "radiochange": function(node) { 
                    ...
                    for(var i = 0; i < olMap.controls.length; i++)
                    {
                      var control = olMap.controls[i];
                      if(control.layers != undefined)
                      {
                        for(var j = 0; j < control.layers.length; j++)
                        {
