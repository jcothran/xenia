...
my $graph_link = '<a href="http://sverdrup.marine.usf.edu/wx/get_graph.php?sensor_id=var1&output=webpage&time_interval=1_day&unit_conversion=en&time_zone_str=EASTERN" target=new onclick="">';
...
foreach my $placemark ($xp->findnodes('//Placemark')) {
	...
	my $local_time = get_local_time($datetime_label, $gmt_time_difference);
        ...
        if ($date_diff_sec > 21600) { $datetime_label = "<span class=\"old\">No data available within the past 6 hours</span>"; }
             ...
             my @line_array = split(/\s+/, timestamp_format(get_local_time($datetime_label, $gmt_time_difference)));
             $datetime_label = "Surface conditions as of ".@line_array[1]." ".@line_array[2]." ".($gmt_time_difference==-5?"EASTERN":"EDT")." on ".@line_array[0] ;
