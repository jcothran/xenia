...
if isstruct(grd_file)
  ...
  for v = varlist
    ...
    try
      ...
      %grd.(vname) = grd.(vname).(vname).data;
      %disp([vname])
      %disp([grd.(vname)])
    catch
      disp(['debug21']) 
  ...
  if isfield(grd,'mask_rho')
    ...
    %land = find(isequal(grd.mask_rho_nan,0));
...
if nargin > 1
  ...
  if ~ischar(scoord)
    ...
    load('sabgom.mat','theta_s');
    %theta_s = theta_tmp(1)
    % theta_s = cell2mat(theta_tmp)
    disp(theta_s)
    %theta_b = nc_varget(scoord,'theta_b');
    load('sabgom.mat','theta_b');
    %Tcline  = nc_varget(scoord,'Tcline');
    load('sabgom.mat','Tcline');
    %N       = length(nc_varget(scoord,'Cs_r'));
    load('sabgom.mat','Cs_r');
    Cs_r = squeeze(Cs_r)
    N = length(Cs_r);
    disp([N])
...
%            ' scoord parameters are input as a vector'])
  ...
  %  rgrid, h', zeta', 0);
  z_r = set_depth(Vtransform, Vstretching, theta_s, theta_b, hc, N, rgrid, h', zeta', 0);
  %z_r = set_depth(Vtransform, Vstretching, theta_s, theta_b, hc, N, rgrid, h.');
  disp(['ok1'])
