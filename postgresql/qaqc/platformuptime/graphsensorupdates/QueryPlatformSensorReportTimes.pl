...
my $strXMLConfigFile = $CommandLineOptions{"XMLConfigFile"};
  ...
  )
{
  die print( "Missing required field(s).\n".
              "Command Line format: --Environment --PlatformID --SensorName --StartDate --EndDate --XMLConfigFile\n". 
              "--Environment\n".
              "--PlatformID .\n".
              "--SensorName \n".
              "--UpdateInterval \n".
...
#load graph info
my $strDBType   = $xp_env->findvalue('//db/type');
...
my $strHTTPDir  = $xp_env->findvalue('//path/http_base');
...
{
  $DB = DBI->connect ("dbi:Pg:dbname=$strDBName","$strDBUser","$strDBPwd");
}
if(!defined $DB) 
{
...
$strHTML .= <<"END_OF_FILE";
<tr><td colspan="4" bgcolor="#ffffff"><img src="$strHTTPDir$strGraphFilename"></td></tr>
END_OF_FILE
...
sub QuerySensorEntryDates
{
  ...
  else
  {
    $strSQL = "SELECT 
                    multi_obs.row_id,
                    ( multi_obs.m_date - interval '-$iTimeZoneShift hours'),
                    multi_obs.row_entry_date,
                    multi_obs.m_value 
                  FROM sensor
                  LEFT JOIN multi_obs on sensor.row_id=multi_obs.sensor_id
                  WHERE 
                    sensor.s_order = $iSOrder                                              AND 
                    sensor.row_id = '$iSensorID'                                           AND
                    m_date >= ( timestamp '$strStartDate' - interval '$iTimeZoneShift hours')  AND 
                    m_date < ( timestamp '$strEndDate' - interval '$iTimeZoneShift hours')     
                  ORDER BY multi_obs.m_date ASC;";
  }
