...
if( $strDBType eq 'sqlite' )
{
  ...
  elsif ($time_query eq 'time_date') { $sql = qq{ SELECT strftime('%m-%d-%Y %H:%M',datetime(substr(m_date,1,19),'-$time_subtract hours')), $column_value FROM $db_table where sensor_id = $sensor_id and $column_value >= $range_min and $column_value <= $range_max and datetime(m_date) >= datetime('$from_date 00:00:00','$time_subtract hours') and datetime(m_date) <= datetime('$to_date 00:00:00','$time_subtract hours') $qc_clause order by m_date; }; }
}
else
{
  if ($time_query eq 'time_last')
  { 
    $sql = "SELECT to_char( ( timestamp m_date - interval '$time_subtract hours'), 'MM-DD-YYYY HH24:MM'), $column_value 
            FROM $db_table 
            WHERE sensor_id = $sensor_id and 
                  m_date > ( now() + interval $time_interval ) and 
                  $column_value >= $range_min and 
                  $column_value <= $range_max $qc_clause 
                  order by m_date;" 
  }
  elsif($time_query eq 'time_date')
  {
